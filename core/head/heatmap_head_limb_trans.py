...
class HeatmapHeadLimb(BaseHead):
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tensor:
        ...
        x_point_top_half = x_point_dict.pred  # [16, 17, 64, 64]

        # x_point = self.final_layer(x_1) # [16, 32, 64, 64] -> [16, 17, 64, 64]

        ##
        # limb
        ##
        x_limb = self.limb_final_layer(x)  # [16, 32, 64, 64] -> [16, 8, 64, 64]
        x_limb_final = x_limb
        # 1.求平均
        # limb_mask = x_limb.mean(dim=1, keepdim=True)  # [16, 8, 64, 64] -> [16, 1, 64, 64]
        # 2.求最大值
        limb_mask = x_limb.max(dim=1, keepdim=True)[0]
        limb_mask = torch.sigmoid(limb_mask)  # 归一化到 [0, 1]

        x_limb = x * limb_mask # [16, 32, 64, 64] * [16, 1, 64, 64] -> [16, 32, 64, 64]
        # x_limb = self.CMAB(x_limb)

        x_limb_dict = self.tokenpose_limb(x_limb)

        x_point_bottom_half = x_limb_dict.pred # [16, 17, 64, 64]

        x_point_final = x_point_top_half + x_point_bottom_half
        # x_point_final = torch.cat([x_point_top_half, x_point_bottom_half], dim=1) # top+bottom -> [16, 17, 64, 64]
        # x_point_final = x_point_top_half

        # token = self.tokenpose(x)
        # x_point = token.pred # [16, 17, 64, 64]

        return x_point_final, x_limb_final
    ...
    def predict(self,
        ...
        if test_cfg.get('flip_test', False):
            ...
            _batch_heatmaps, _limb = self.forward(_feats)

            flip,_ = self.forward(_feats_flip)
            ...
            _point, _limb = self.forward(feats)

        preds = self.decode(_point)

        if test_cfg.get('output_heatmaps', False):
            pred_fields = [
                PixelData(heatmaps=hm) for hm in _limb.detach() # _point or _limb
            ]
    ...
    def loss(self,
        ...
        pred_fields, pred_fields_limb = self.forward(feats)
